...
#             logger.info(f"✅ Found {service_name} key in HuggingFace secrets")
...
#             if key and key.strip():
#                 keys.append(key.strip())
#                 logger.info(f"✅ Found {service_name} key in environment")
#                 break
...
#                     if key and key.strip():
#                         keys.append(key.strip())
#                         logger.info(f"✅ Found {service_name} key in Streamlit secrets")
#                         break
#         except Exception:
#             pass
...
def get_api_keys(service_name: str, key_names: List[str]) -> List[str]:
    ...
    for i in range(1, 6):  # Check for HF_GROQ_API_KEY_1 through HF_GROQ_API_KEY_5
        ...
        if key and key.strip():
            keys.append(key.strip())
            logger.info(f"✅ Found {service_name} key {i} in HuggingFace secrets")
    ...
    if not keys:
        ...
        logger.info(f"✅ Found {len(keys)} {service_name} API keys")
